...
def _resolve_culane_eval_cmd():
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    candidate_paths = [
        os.path.join(repo_root, "evaluation", "culane", "evaluate"),
        os.path.join(repo_root, "evaluation", "culane", "src", "evaluate"),
    ]
    ...
    culane_dir = os.path.join(repo_root, "evaluation", "culane")
    try:
        subprocess.run(["make"], cwd=culane_dir, check=True)
    except Exception as e:
        raise FileNotFoundError(
            "CULane evaluator binary not found and auto-build failed. "
            "Please run `make -C evaluation/culane` and ensure OpenCV build dependencies are installed. "
            f"Original error: {e}"
        )
    ...
    raise FileNotFoundError(
        "CULane evaluator binary not found after build. "
        "Expected one of: evaluation/culane/evaluate or evaluation/culane/src/evaluate"
    )
...
def call_culane_eval(data_dir, exp_name, output_path):
    ...
    eval_cmd = _resolve_culane_eval_cmd()
    ...
    if not os.path.exists(out0):
        raise FileNotFoundError(
            "CULane evaluation output was not generated. "
            "Please verify evaluator build and dataset list/test_split files."
        )
