import torch
from utils.loss import ParsingRelationDis, ParsingRelationLoss, SoftmaxFocalLoss
from utils.metrics import AccTopk, Metric_LaneIoU, Metric_mIoU, MultiLabelAcc
    ...
    if cfg.use_aux:
        metric_dict = {
            ...
            "op": [
                ...
                Metric_LaneIoU(cfg.num_lanes + 1),
            ],
            "data_src": [
                ("cls_out", "cls_label"),
                ("cls_out", "cls_label"),
    ...
    def step(self, external_iter=None):
        ...
        for group, lr in zip(self.optimizer.param_groups, self.base_lr):
            group["lr"] = (
                self.eta_min
                + (lr - self.eta_min)
                * (1 + math.cos(math.pi * self.iters / self.T_max))
                / 2
            )
